...
BitWriter::BitWriter(const std::string& filename, bool append) : current_byte(0), bit_pos(0) {
...
void BitWriter::WriteBitSequence(const std::vector<unsigned char>& data, size_t bitCount) {
  size_t bits_written = 0;
  size_t byte_idx = 0;
  size_t bit_idx_in_byte = 0;

  while (bits_written < bitCount) {
    unsigned char input_byte = (byte_idx < data.size()) ? data[byte_idx] : 0;
...
BitReader::BitReader(const std::string& filename, size_t skipBytes) : current_byte(0), bit_pos(8) {
  in.open(filename, std::ios::binary);
  if (!in) {
    std::cerr << "Помилка відкриття файлу для читання: " << filename << "\n";
    return;
  }
  if (skipBytes > 0) in.seekg(static_cast<std::streamoff>(skipBytes));
}
...
int BitReader::ReadBit() {
  ...
  int bit = (current_byte >> (7 - bit_pos)) & 1;
  bit_pos++;
  return bit;
}
...
std::vector<unsigned char> BitReader::ReadBitSequence(size_t bitCount) {
  ...
  if (out_bit_pos > 0) result.push_back(out_byte);
...
// ================= Демонстрація (з лабораторної 3) =================

static void printHex(const std::vector<unsigned char>& vec, const std::string& name) {
  std::cout << name << " = [";
  for (size_t i = 0; i < vec.size(); ++i) {
    std::cout << std::hex << std::uppercase << std::setw(2) << std::setfill('0') << static_cast<int>(vec[i]);
