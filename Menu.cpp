...
int main() {
  ...
  while (true) {
    ...
    std::cout << "6) Huffman encode file\n";
    std::cout << "7) Huffman decode file\n";
    ...
    auto promptFiles = [&](std::string& in, std::string& out, const char* inLabel) {
      std::cout << inLabel << ": ";
      std::cin >> in;
      std::cin.ignore();
      std::cout << "Вихідний файл (Enter = за замовчуванням): ";
    ...
    if (choose == 1) {
      ...
      std::string in, out;
      promptFiles(in, out, "Вхідний файл");
      HuffmanEncodeFile(in, out);

    } else if (choose == 7) {
      std::string in, out;
      promptFiles(in, out, "Закодований файл");
